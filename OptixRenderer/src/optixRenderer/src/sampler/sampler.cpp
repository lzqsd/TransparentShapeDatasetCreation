    ...
    for(int r = 0; r < height; r++){
        for(int c = 0; c < width; c++){
            ...
            for(int ch = 0; ch < 3; ch++){ 
                ...
                imgData[14*(r*width + c) + ch + offset1] = sum1 / N;
                imgData[14*(r*width + c) + ch + offset2] = sum2 / N;
                imgData[14*(r*width + c) + ch + offset4] = sum4 / N;
                imgData[14*(r*width + c) + ch + offset5] = sum5 / N;
            }
            ...
            imgData[14*(r*width + c) + offset3 ] = sum3 / N;
            imgData[14*(r*width + c) + offset6 ] = sum6 / N;
        }
    }
    imgBuffer1 -> unmap();
    imgBuffer2 -> unmap();
