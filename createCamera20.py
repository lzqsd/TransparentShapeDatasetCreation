...
parser.add_argument('--imNum', type=int, default=20, help='the number of camera view')
...
thetaValue = np.array([50, 100], dtype=np.float32 )
phiValue = np.array([0, 36, 72, 108, 144, 180, 216, 252, 288, 324], dtype=np.float32 )
...
for n in range(rs, min(re, len(shapes)) ):
    ...
    if osp.isfile( osp.join(shapeRoot, 'cam20.txt' ) ):
    ...
    with open(osp.join(shapeRoot, 'cam20.txt'), 'w') as camOut:
        camOut.write('%d\n' % imNum )
